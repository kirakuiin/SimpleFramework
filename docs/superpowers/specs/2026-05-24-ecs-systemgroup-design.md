...
- Let callers update all systems in a group through `SystemGroup.Update()`.
...
`Add(system)` uses order `0`. `Add(system, order)` stores both the explicit order and an internal insertion index. `Update()` executes systems sorted by `order` ascending, then by insertion index ascending.
...
Changing the group while `Update()` is running is invalid. Calling `Add` or `Remove` during group update throws `InvalidOperationException`. This mirrors the existing ECS rule that query traversal rejects structural changes during enumeration.
...
- Exceptions thrown by a system's `Update()` are not swallowed by `SystemGroup`.
